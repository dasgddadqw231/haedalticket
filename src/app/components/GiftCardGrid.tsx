...
import { getRates, type Rate } from "../lib/db";

const giftCards = [
  { id: 1, cardId: "culture", name: "컬쳐랜드 상품권", img: cultureCardImg },
  { id: 2, cardId: "starbucks", name: "스타벅스 상품권", img: starbucksCardImg },
  { id: 3, cardId: "lotte", name: "롯데모바일상품권", img: lotteCardImg },
];
...
export function GiftCardGrid({ onSelectCard }: { onSelectCard?: (cardId: string) => void }) {
  ...
  return (
    <section className="px-4 relative z-10" style={{ marginTop: "2.5rem" }}>
        ...
        {giftCards.map((card) => (
          ...
          >
            ...
            <div className="text-right shrink-0">
              <span className="text-[#1E2A5E] text-sm">{rates.find((r) => r.key === card.cardId)?.value ?? "--"}%</span>
